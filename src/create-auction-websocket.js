...
async function processIncomingMessage(data) {
    ...
    try {
        ...
        if (messageObj?.params?.result?.transaction?.meta?.logMessages[1] !== 'Program log: Instruction: CreateAuction' && messageObj.params) { // change to === in prod
            ...
            (async () => {
                const conn = new Connection(clusterApiUrl('devnet')); // Devnet
                // const conn = new Connection(clusterApiUrl('mainnet')); // Mainnet
                ...
                if (tx) {
                    ...
                    if (strMint.length > 6) {
                        ...
                        (async () => {
                            ...
                            const solscan = hyperlink('Solscan', `https://solscan.io/tx/${signature}?cluster=devnet`);
                            const solanafm = hyperlink('SolanaFM', `https://solana.fm/tx/${signature}?cluster=devnet`);
                            const solscanMint = hyperlink(truncatedMint, `https://solscan.io/token/${dataObject?.mint}?cluster=devnet`);
                            ...
                            const createAuction = new EmbedBuilder()
                                .setTitle(BOT)
                                .setDescription("_<a:love:1196257367805939712> A new auction has been created!_") // can change the emoji when inside the said service, right click on the emoji, copy text
                                .setColor('#fee185')
                                .setImage(`${mainImg}`)
                                .addFields(
